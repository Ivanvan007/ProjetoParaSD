...
let DATA_DIR = path.join(__dirname, 'DB-data'); // data directory path
...
function setupServer(port) {
    ...
    serverApp.post('/db/c', (req, res) => {
        ...
        filepath = path.join(DATA_DIR, NODE_ID, masterId, filename);
...
async function conductElection(myNodeId) {
    let promises = NODES.filter(node => node.id > myNodeId)
        .map(node => axios.get(`${node.url}/election`).catch(err => null)); // Send election request to higher nodes

    let results = await Promise.all(promises); // Wait for all responses
    let stillActive = results.some(result => result && result.status === 200); // Check if any node is still active, i.e., has not become the master
